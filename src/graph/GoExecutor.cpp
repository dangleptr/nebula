            ...
            for (auto& edata : vdata.edge_data) {
                for (auto& edge : edata.get_edges()) {
                    ...
                    if (!schemas.empty()) {
                        ...
                        EdgeRanking rank;
                        auto rc = reader->getVid(_RANK, rank);
...
bool GoExecutor::onlyStructure() {
    return whereWrapper_->filter_  == nullptr
            && edgeTypes_.size() == 1
            && (yields_.empty()
                || (yields_.size() == 1 && yields_[0]->expr()->isEdgeDstIdExpression()));
}
...
StatusOr<std::vector<cpp2::RowValue>> GoExecutor::toThriftResponse(RpcResponse&& rpcResp) {
    ...
    if (onlyStructure()) {
        auto uniqResult = std::make_unique<std::unordered_set<VertexID>>();
            ...
            for (auto& vdata : resp.vertices) {
                ...
                for (auto& edata : vdata.edge_data) {
                    for (auto& edge : edata.edges) {
                        // Check if duplicate
                        if (distinct_) {
                            auto ret = uniqResult->emplace(edge.get_dst());
                            if (!ret.second) {
                                continue;
                            }
                        }
...
bool GoExecutor::processFinalResult(RpcResponse &rpcResp, Callback cb) const {
    ...
    for (auto *column : yields_) {
        colTypes.emplace_back(calciteExprType(column->expr()));
    }
    for (auto &resp : all) {
        if (resp.get_vertices() == nullptr) {
            continue;
...
nebula::cpp2::SupportedType GoExecutor::calciteExprType(Expression* exp) const {
    auto spaceId = ectx()->rctx()->session()->space();
    switch (exp->kind()) {
